...
import re
import json
from jd_bingxiang.items import JdBingxiangItem
import time,random
import pandas as pd
import csv

class jdspider(scrapy.Spider):
    name="jd_bingxiang"                 #京东电冰箱
    allowed_domains=["jd.com"]
    start_urls=[
        "https://list.jd.com/list.html?cat=737,794,878"   #京东电冰箱
    ]
    num=0
    pagenum=0
    ...
    def goods(self,response):
        ...
        if "error" in url or "2017?t" in url:        #302重定向页面,写回原页面处理
            url="https://item.jd.com/"+str(ProductID)+".html"
            item = JdBingxiangItem(ProductID=ProductID,PreferentialPrice=PreferentialPrice,price=price)
        ...
        # --------------------全球购网页---------------------------------------------
            ...
            try:
                ...
                try:
                    X_type = re.findall(">门款式：(.*?)<", detail_1[0])[0].strip()
                except:
                    X_type = None
            ...
            try:
                door_material = x['面板材质']
            except:
                try:
                    door_material = re.findall(">面板材质：(.*?)<", detail_1[0])[0].strip()
                except:
                    door_material = None
            ...
            try:
                ...
                try:
                    cooling = re.findall(">制冷方式：(.*?)<", detail_1[0])[0].strip()
                except:
                    cooling = None
            ...
            try:
                ...
                try:
                    frequency = re.findall(">压缩机：(.*?)<", detail_1[0])[0].strip()
                except:
                    frequency = None

            try:
                level = x['能效等级'].replace(' ','')
            except:
                try:
                    level = re.findall(">能效等级：(.*?)<", detail_1[0])[0].strip().replace(' ','')
                except:
                    level = None

            try:
                temp_control = re.findall(">控温方式：(.*?)<", detail_1[0])[0].strip()
            except:
                try:
                    temp_control = x['控制方式']
                except:
            ...
            try:
                ...
                try:
                    display = re.findall(">显示方式：(.*?)<", detail_1[0])[0].strip()
                except:
                    display = None
            ...
            try:
                ...
                try:
                    refrigerant = re.findall(">制冷剂：(.*?)<", detail_1[0])[0].strip()
                except:
                    refrigerant = None
            ...
            try:
                ...
                try:
                    defrost = re.findall(">除霜模式：(.*?)<", detail_1[0])[0].strip()
                except:
                    defrost = None
            ...
            try:
                ...
                try:
                    sound = re.findall(">运转音：(.*?)<", detail_1[0])[0].strip()
                except:
                    sound = None
            ...
            try:
                ...
                try:
                    color = re.findall(">面板颜色：(.*?)<", detail_1[0])[0].strip()
                except:
                    color = None
        ...
        # ---------------------普通网页-----------------------------------
            ...
            try:
                cooling = x['制冷方式'].replace(' ','')
            except:
                try:
                    cooling = re.findall(">制冷方式：(.*?)<", detail_1[0])[0].strip().replace(' ','')
                except:
                    cooling = None
            ...
            try:
                ...
                try:
                    level = re.findall(">能效等级：(.*?)<", detail_1[0])[0].strip()
                except:
                    level = None
        ...
        if float(PreferentialPrice)>0.00:
            ...
            item['door_material']=door_material
            item['cooling']=cooling
            item['frequency']=frequency
            item['level']=level
            item['temp_control'] = temp_control
            item['display'] = display
            item['refrigerant'] = refrigerant
            item['defrost'] = defrost
            item['sound'] = sound
            item['color'] = color
